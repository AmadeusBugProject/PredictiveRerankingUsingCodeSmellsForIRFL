...
def main():
    for bootstrap_sample_path in glob.glob(root_dir() + 'p_model_for_smell_classification_bootstrap/*'):
        ranking_training_df = pandas.read_csv(bootstrap_sample_path + '/ranknet_train_df.csv')
        predictions_train = pandas.read_csv(bootstrap_sample_path + '/05_train__predictions.csv')

        k = bootstrap_sample_path.split('/')[-1]
        Path(path + k).mkdir(parents=True, exist_ok=True)

        ranking_performance_train_df = ranking(ranking_training_df, predictions_train)
        ranking_performance_train_df.to_csv(path + k + '/ranking_performance_train_df.csv')

    # rank_test_samples_df = pandas.read_csv(path + 'rank_test_samples_df.csv')
    #
    # rank_test_samples_df = rank_test_samples_df.fillna(0)
    #
    # summary = rank_test_samples_df[['top_1', 'top_5', 'top_10', 'rr', 'ap', 'label']].groupby(by='label').mean()
    # summary.to_csv(path + 'bug_scores_k' + str(CUT_OFF) + '_summary.csv')


def ranking(ranking_training_df, predictions_train):
    ...
    for i, version_files_csv in enumerate(pmd_paths):
        ...
        if version_id in pred_versions:
            ...
            for index, row in version_df.iterrows():
                for tool in TOOLS:
                    bug_score_results = rank_and_score_bug(version_id, row.to_dict(), pmd_files_df, ground_truth_files_df, tool)
...
def rank_and_score_bug(version_id, bug_row, pmd_files_df, ground_truth_files_df, tool):
    ...
    base_ranking.update({'label': tool, 'tool': tool, 'bug_id': bug_row['bug_id'], 'version_id': bug_row['version_id'], 'orig_weight': 1})

    rankings = [base_ranking]
    for orig_weight in numpy.arange(0.7, 1, 0.01):
        ...
        new_ranking.update({'label': tool + '_' + 'mean_lse' + '_' + str(orig_weight), 'tool': tool, 'bug_id': bug_row['bug_id'], 'version_id': bug_row['version_id'], 'orig_weight': orig_weight})
